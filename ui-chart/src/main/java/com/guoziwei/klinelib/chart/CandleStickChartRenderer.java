    ...
    protected void drawDataSet(Canvas c, ICandleDataSet dataSet) {
        ...
        for (int j = mXBounds.min; j <= mXBounds.range + mXBounds.min; j++) {
            ...
            if (showCandleBar) {
                ...
                // 防止top和bottom太小，画不出来
                float delta = Math.abs(mBodyBuffers[3]-mBodyBuffers[1]);
                // draw body differently for increasing and decreasing entry
                if (open > close && delta >0.7) { // decreasing
                    ...
                    c.drawRect(
                            ...
                            mRenderPaint);
                    Log.d("leon", "CandleStickChartRenderer  open > close drawDataSet: mBodyBuffers[0]:"+ DECIMAL_FORMAT_6_DIGITS_ROUND_DOWN.format(mBodyBuffers[0]) +" mBodyBuffers[1]:"+DECIMAL_FORMAT_6_DIGITS_ROUND_DOWN.format(mBodyBuffers[1])
                            +" mBodyBuffers[2]:"+DECIMAL_FORMAT_6_DIGITS_ROUND_DOWN.format(mBodyBuffers[2])+" mBodyBuffers[3]:"+DECIMAL_FORMAT_6_DIGITS_ROUND_DOWN.format(mBodyBuffers[3])+" barSpace:"+barSpace
                            +" open:"+DECIMAL_FORMAT_6_DIGITS_ROUND_DOWN.format(open)+" close:"+DECIMAL_FORMAT_6_DIGITS_ROUND_DOWN.format(close));
                } else if (open < close  && delta >0.7) {
    ...
    public void drawValues(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            if (maxIndex > minIndex) {
                //画右边
                String highString = "← " + dataSet.getValueFormatter().getFormattedValue(minValue,null,i,mViewPortHandler);
...
//                String highString = Float.toString(minValue) + " →";
            ...
            if (maxIndex > minIndex) {
                //画左边
                String highString = dataSet.getValueFormatter().getFormattedValue(maxValue,maxEntry,i,mViewPortHandler)+" →";
