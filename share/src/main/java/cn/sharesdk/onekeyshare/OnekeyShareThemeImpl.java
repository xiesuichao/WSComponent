...
import com.wanshare.wscomponent.share.R;
...
public abstract class OnekeyShareThemeImpl implements PlatformActionListener, Callback {
	...
	final boolean formateShareData(Platform plat) {
		...
		boolean isFacebook = "Facebook".equals(name);
		if(isFacebook && !plat.isClientValid()){
			toast(context.getString(R.string.no_facebook_client_hint));
			return false;
		}
